...
📝 Next Steps (DEPRECATED - Use setup wizard instead):

  1. Start the application: pnpm dev
  2. Visit http://localhost:3000/admin/setup
  3. Complete the setup wizard to create superadmin
  4. API keys are now managed through the admin panel
...
⚠️  IMPORTANT: Store the API keys above securely!
   ...
   NEXT_PUBLIC_ADMIN_PANEL_API_KEY=<admin_panel_api_key>
...
### **Step 3: Update `.env` File** (DEPRECATED)

⚠️ **This step is outdated.** The project now uses a setup wizard instead of manual .env configuration for admins.

**Modern approach:**
1. Run `pnpm dev`
2. Visit `http://localhost:3000/admin/setup`
3. Enter your admin email in the setup wizard
4. API keys are automatically generated

**Old approach (for reference only):**
```bash
# This is deprecated - use setup wizard instead
SUPER_ADMIN_EMAIL=your-email@gmail.com  # ❌ No longer used
NEXT_PUBLIC_ADMIN_PANEL_API_KEY=<key>    # ✅ Generated by setup wizard
```
...
**Expected output:**
```
[API] Server listening at http://localhost:1337
...
```bash
curl http://localhost:1337/api/admin/collections
```
...
curl -H "X-API-Key: invalid-key-12345" \
     http://localhost:1337/api/admin/collections
```
...
curl -H "X-API-Key: YOUR_ADMIN_PANEL_API_KEY" \
     -H "Authorization: Bearer YOUR_JWT_TOKEN" \
     "http://localhost:1337/api/admin/collections/users/data?page=1&limit=10"
```
...
curl -H "X-API-Key: YOUR_ADMIN_PANEL_API_KEY" \
     -H "Authorization: Bearer YOUR_JWT_TOKEN" \
     "http://localhost:1337/api/admin/collections/users/data?search=system"
```
...
curl -H "X-API-Key: YOUR_ADMIN_PANEL_API_KEY" \
     -H "Authorization: Bearer YOUR_JWT_TOKEN" \
     http://localhost:1337/api/admin/api-keys
```
...
curl -H "X-API-Key: YOUR_ADMIN_PANEL_API_KEY" \
     -H "Authorization: Bearer YOUR_JWT_TOKEN" \
     http://localhost:1337/api/admin/api-keys/stats
```
...
curl -X POST \
     ...
     -d '{"name":"test_api_key"}' \
     http://localhost:1337/api/admin/api-keys/generate
```
...
curl -X POST \
     -H "X-API-Key: YOUR_ADMIN_PANEL_API_KEY" \
     -H "Authorization: Bearer YOUR_JWT_TOKEN" \
     http://localhost:1337/api/admin/api-keys/1/revoke
```
