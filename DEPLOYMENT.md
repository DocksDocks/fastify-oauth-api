...
⚠️ **DEPRECATED:** Admin configuration is now managed through the setup wizard and `authorized_admins` database table.

| Variable | Description | Status | Notes |
|----------|-------------|--------|-------|
| `ADMIN_EMAIL` | ~~Primary admin email~~ | ❌ Deprecated | Use setup wizard at `/admin/setup` |
| `ADMIN_EMAILS_ADDITIONAL` | ~~Additional admin emails~~ | ❌ Deprecated | Manage via "Authorized Admins" in admin panel |
| `SUPER_ADMIN_EMAIL` | ~~Super admin email(s)~~ | ❌ Deprecated | First user in setup wizard becomes superadmin |

**Modern approach:**
1. First deployment: Access `/admin/setup` to create superadmin
2. Additional admins: Add emails via "Authorized Admins" page (superadmin only)
3. Auto-promotion: Users with authorized emails are promoted on first OAuth login
